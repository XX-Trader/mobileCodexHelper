...
const MessageComponent = memo(({ message, prevMessage, createDiff, onFileOpen, onShowSettings, onGrantToolPermission, autoExpandTools, showRawParameters, showThinking, selectedProject, provider }: MessageComponentProps) => {
  ...
  const [isCommandGroupOpen, setIsCommandGroupOpen] = useState(false);
  ...
  const commandGroupMessages = useMemo(
    () => (Array.isArray(message.commandGroupMessages) ? message.commandGroupMessages : []),
    [message.commandGroupMessages],
  );
  const commandGroupSummary = useMemo(() => {
    if (!message.isCommandGroup || commandGroupMessages.length === 0) {
      return null;
    }

    const latestCommand = [...commandGroupMessages]
      .reverse()
      .map((commandMessage) => {
        const toolInput = commandMessage.toolInput;
        ...
        if (
          ...
          return (toolInput as { command: string }).command.trim();
        }

        return '';
      })
      .find((commandText) => commandText.length > 0);

    return {
      count: commandGroupMessages.length,
      latestCommand: latestCommand || 'shell command',
    };
  }, [commandGroupMessages, message.isCommandGroup]);
  ...
  return (
    <div
      ...
      data-message-type={message.type || undefined}
      ...
      {message.type === 'user' ? (
        ...
        <div className="flex w-full items-end space-x-0 sm:w-auto sm:max-w-[85%] sm:space-x-3 md:max-w-md lg:max-w-lg xl:max-w-xl">
          <div className="group flex-1 rounded-2xl rounded-br-md bg-blue-600 px-3 py-2 text-white shadow-sm sm:flex-initial sm:px-4">
            <div className="relative">
              ...
              {shouldCollapseUserMessage && !isUserMessageExpanded && (
                <div className="pointer-events-none absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-blue-600 via-blue-600/85 to-transparent" />
              )}
            </div>
            ...
            {shouldCollapseUserMessage && (
              <div className="mt-2 flex justify-end">
                <button
                  type="button"
                  onClick={() => setIsUserMessageExpanded((current) => !current)}
        ...
        <div className="w-full">
          ...
          <div className="w-full">
            ...
            {message.isCommandGroup ? (
              <div className="my-1 rounded-xl border border-emerald-200/70 bg-emerald-50/40 p-2 dark:border-emerald-900/50 dark:bg-emerald-950/20">
                <button
                  type="button"
                  onClick={() => setIsCommandGroupOpen((current) => !current)}
                  className="flex w-full items-center gap-2 text-left"
                >
                  <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-300">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 items-center gap-2">
                      <span className="flex-shrink-0 text-xs font-semibold text-emerald-700 dark:text-emerald-300">
                        {commandGroupSummary?.count || 0} commands
                      </span>
                      <span className="truncate font-mono text-xs text-emerald-700/80 dark:text-emerald-200/80">
                        {commandGroupSummary?.latestCommand}
                      </span>
                    </div>
                  </div>
                  <svg
                    className={`h-4 w-4 flex-shrink-0 text-emerald-600 transition-transform dark:text-emerald-300 ${
                      isCommandGroupOpen ? 'rotate-90' : ''
                    }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                ...
                {isCommandGroupOpen && (
                  <div className="mt-2 space-y-2 border-t border-emerald-200/60 pt-2 dark:border-emerald-900/50">
                    {commandGroupMessages.map((commandMessage, commandIndex) => (
                      <div key={`${commandMessage.toolId || commandMessage.timestamp}-${commandIndex}`} className="space-y-1">
                        <ToolRenderer
                          toolName={commandMessage.toolName || 'Bash'}
                          toolInput={commandMessage.toolInput}
                          toolResult={commandMessage.toolResult}
                          toolId={commandMessage.toolId}
                          mode="input"
                          ...
                          rawToolInput={typeof commandMessage.toolInput === 'string' ? commandMessage.toolInput : undefined}
                        />
                        {commandMessage.toolResult &&
                          !shouldHideToolResult(commandMessage.toolName || 'Bash', commandMessage.toolResult) && (
                            <div id={`tool-result-${commandMessage.toolId}`} className="scroll-mt-4">
                              <ToolRenderer
                                toolName={commandMessage.toolName || 'Bash'}
                                toolInput={commandMessage.toolInput}
                                toolResult={commandMessage.toolResult}
