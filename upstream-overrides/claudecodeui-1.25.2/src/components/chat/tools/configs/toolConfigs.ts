...
const getBashCommandText = (input: unknown): string => {
...
const getBashCommandSummary = (input: unknown): string => {
  const commandText = getBashCommandText(input).trim();
  if (!commandText) {
    return 'shell command';
  }

  const firstLine = commandText.split(/\r?\n/, 1)[0].trim();
  return firstLine || 'shell command';
};
...
export const TOOL_CONFIGS: Record<string, ToolDisplayConfig> = {
  ...
  Bash: {
    input: {
      type: 'collapsible',
      title: (input) => getBashCommandSummary(input),
      defaultOpen: false,
      contentType: 'text',
      getContentProps: (input) => ({
        content: getBashCommandText(input),
        format: 'code',
      }),
    },
    result: {
