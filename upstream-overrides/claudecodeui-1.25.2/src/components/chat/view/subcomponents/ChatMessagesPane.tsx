...
export default function ChatMessagesPane({
  ...
  const renderedMessages = useMemo(() => {
    // Keep tool messages structurally stable while a turn is in progress.
    // Retroactively regrouping Bash commands into a synthetic summary causes
    // the list item height to shrink/expand and makes the chat scroll jump.
    ...
    >
      {isLoadingSessionMessages && chatMessages.length === 0 ? (
        ...
        <>
          ...
          {renderedMessages.map(({ message, renderKey }, index) => {
