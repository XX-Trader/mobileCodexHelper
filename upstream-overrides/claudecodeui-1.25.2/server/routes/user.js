...
router.get('/language', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const preference = userDb.getPreferredLanguageRecord(userId);

    res.json({
      success: true,
      language: preference.language,
      isExplicitlySet: preference.isExplicitlySet,
    });
  } catch (error) {
    console.error('Error getting preferred language:', error);
    res.status(500).json({ error: 'Failed to get preferred language' });
  }
});
...
router.put('/language', authenticateToken, async (req, res) => {
  try {
    ...
    if (!SUPPORTED_PREFERRED_LANGUAGES.has(language)) {
      return res.status(400).json({ error: 'Unsupported language' });
    }

    const savedLanguage = userDb.updatePreferredLanguage(userId, language);
