...
import { resolvePendingViewSessionId } from '../utils/pendingSession';
...
interface UseChatRealtimeHandlersArgs {
  ...
  setStatusStartedAt: Dispatch<SetStateAction<number | null>>;
  refreshTokenBudget?: (
    sessionIdOverride?: string | null,
    providerOverride?: SessionProvider | string,
  ) => Promise<void>;
...
function keepEarliestStartedAt(
  setStatusStartedAt: Dispatch<SetStateAction<number | null>>,
  startedAt: unknown,
) {
  ...
  setStatusStartedAt((previousStartedAt) => {
    ...
    return Math.min(previousStartedAt, normalizedStartedAt);
  });
}

function getActiveSessionStartedAt(
  sessions: unknown,
  provider: SessionProvider,
  sessionId: string | null | undefined,
): number | null {
  if (!sessions || typeof sessions !== 'object' || !sessionId) {
    return null;
  }

  const providerSessions = (sessions as Record<string, unknown>)[provider];
  if (!Array.isArray(providerSessions)) {
    return null;
  }

  const matchedSession = providerSessions.find((currentSession) => {
    if (!currentSession || typeof currentSession !== 'object') {
      return false;
    }

    return (currentSession as { id?: unknown }).id === sessionId;
  });

  if (!matchedSession || typeof matchedSession !== 'object') {
    return null;
  }

  return normalizeStartedAt((matchedSession as { startedAt?: unknown }).startedAt);
}
...
function resolveRealtimeSessionId(
  ...
  return resolveViewScopedSessionId(selectedSessionId, currentSessionId, pendingViewSessionId);
}
...
export function useChatRealtimeHandlers({
  ...
  useEffect(() => {
    ...
    const activeViewSessionId = resolveRealtimeSessionId(
      provider,
    ...
    const scheduleTokenBudgetRefresh = (
      sessionIdOverride?: string | null,
      providerOverride?: SessionProvider | string,
    ) => {
      if (!refreshTokenBudget) {
        return;
      }

      const targetSessionId =
        sessionIdOverride || activeViewSessionId || selectedSession?.id || currentSessionId || null;
      if (!targetSessionId) {
        return;
      }

      window.setTimeout(() => {
        void refreshTokenBudget(targetSessionId, providerOverride || provider);
      }, providerOverride === 'codex' || provider === 'codex' ? 180 : 0);
    };

    const scheduleProjectsRefresh = (delay = 0) => {
    ...
    const resolveTemporaryViewSessionId = () => {
      ...
      return (
        candidateSessionIds.find(
          (sessionId) =>
            ...
            sessionId.startsWith(TEMPORARY_CODEX_SESSION_PREFIX),
        ) || null
      );
    };
    ...
    const finalizeLifecycleForCurrentView = (...sessionIds: Array<string | null | undefined>) => {
      const resolvedSessionIds = collectSessionIds(...sessionIds, pendingSessionId, pendingViewSessionId);
    ...
    switch (latestMessage.type) {
      case 'session-created':
        if (
          ...
          (
            !selectedSession?.id ||
            Boolean(resolveTemporaryViewSessionId()) ||
            !currentSessionId ||
            currentSessionId.startsWith(TEMPORARY_CODEX_SESSION_PREFIX)
          )
        ) {
          const temporaryViewSessionId = resolveTemporaryViewSessionId();
          ...
          onReplaceOptimisticSession?.(temporaryViewSessionId, latestMessage.sessionId);
        ...
        if (latestMessage.data) {
          if (provider === 'codex') {
            if (
              ...
              latestMessage.sessionId.startsWith(TEMPORARY_CODEX_SESSION_PREFIX)
            ) {
              setTokenBudget(latestMessage.data);
            } else {
              scheduleTokenBudgetRefresh(latestMessage.sessionId, 'codex');
            }
          } else {
            setTokenBudget(latestMessage.data);
          }
      ...
      case 'codex-complete': {
        ...
        if (codexActualSessionId) {
          scheduleProjectsRefresh(0);
          scheduleProjectsRefresh(250);
          void reloadSessionHistory?.(codexActualSessionId, 'codex');
        }
        scheduleTokenBudgetRefresh(
          codexActualSessionId || codexCompletedSessionId,
          'codex',
        );
        break;
      }
      ...
      case 'codex-error':
        ...
        if (latestMessage.sessionId) {
          scheduleProjectsRefresh(0);
          void reloadSessionHistory?.(latestMessage.sessionId, 'codex');
        }
        setChatMessages((previous) => [
          ...previous,
          {
      ...
      case 'session-status': {
        ...
        if (latestMessage.isProcessing) {
          ...
          if (isCurrentSession) {
            keepEarliestStartedAt(setStatusStartedAt, latestMessage.startedAt);
            setIsLoading(true);
            setCanAbortSession(true);
          }
          break;
        ...
        if (isCurrentSession) {
          clearLoadingIndicators();
          clearPendingViewSession(statusSessionId);
        }
        break;
      }

      case 'active-sessions': {
        keepEarliestStartedAt(
          setStatusStartedAt,
          getActiveSessionStartedAt(latestMessage.sessions, provider, activeViewSessionId),
        );
        break;
      }
      ...
      case 'claude-status': {
        ...
        keepEarliestStartedAt(setStatusStartedAt, statusData.startedAt ?? latestMessage.startedAt);
        setIsLoading(true);
      ...
      case 'error':
        ...
        setChatMessages((previous) => [
          ...
          {
            type: 'error',
            content: latestMessage.error || latestMessage.message || 'An unexpected backend error occurred',
            timestamp: new Date(),
          },
        ]);
        break;
